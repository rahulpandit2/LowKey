  ...
  const [helpPosts] = useState<HelpPost[]>([
    {
      ...
      author: 'Anonymous',
      preview:
        'Struggling with work-life balance after promotion. Need advice on setting boundaries...',
      responseCount: 7,
      status: 'active',
          ...
          {['all', 'active', 'resolved', 'escalated'].map((status) => (
            <button
              ...
              className={`text-xs uppercase tracking-widest px-4 py-2 rounded-sm transition-colors ${
                filter === status ? 'bg-primary/10 text-primary' : 'text-zinc-500 hover:text-white'
              }`}
            >
              {status}
            </button>
